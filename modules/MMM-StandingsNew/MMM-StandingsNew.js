...
Module.register("MMM-StandingsNew",{
	...
	getData: function (clearAll) {
		...
		for (var i = 0; i < this.config.sports.length; i++) {
			...
			this.sendSocketNotification("STANDINGS_RESULT_NEW-" + this.config.sports[i].league, this.config.url + sport);
		}
	},

	socketNotificationReceived: function(notification, payload) {
		if (notification.startsWith("STANDINGS_RESULT_NEW")) {
