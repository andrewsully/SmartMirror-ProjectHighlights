...
Module.register("MMM-CalendarExtMinimonth",{
  ...
  drawSlot: function() {
    ...
    for (var i = 0; i < this.slots.length; i++) {
      ...
      if (slot.events.length > 0 && this.names.length > 0) {
        // Find the first matching calendar
        let calName = null;
        for (let ev of slot.events) {
          if (this.names.includes(ev.calendarName)) {
        ...
        if (calName) {
          const borderVar = `--calendar-${calName.toLowerCase()}-border`;
          cell.style.border = `2px solid var(${borderVar})`;
        }
      }
      week.appendChild(cell)
      if (6 == (i % 7)) {
