...
Module.register('MMM-ScoreboardNew', {
  ...
  defaults: {
    ...
    rolloverDays: 3, // days to show previous games for NFL (overrides rolloverHours for NFL)
    ...
    showWeekLabel: true, // Show NFL week number header
    wallberryStyle: false, // Use WB-clock styling theme
    sports: [
      {
        league: 'NHL',
        teams: ['TOR'],
  ...
  setupScrollAnimation: function (wrapper) {
    ...
    const prevScrollContainer = document.querySelector('.MMM-ScoreboardNew .scroll-container'),
      domHeight = prevScrollContainer ? prevScrollContainer.scrollHeight : document.querySelector('.MMM-ScoreboardNew .wrapper').scrollHeight
  ...
  getDom: function () {
    ...
    */
    ...
    // Week label will be added after the scores
    ...
    */
    ...
    Object.keys(this.sportsData).forEach((sport) => {
      if (this.sportsData[sport]['scores'] && this.sportsData[sport]['scores'].length > 0) {
        anyGames = true
      }
    })
    Object.keys(this.sportsDataYd).forEach((sport) => {
      if (this.sportsDataYd[sport]['scores'] && this.sportsDataYd[sport]['scores'].length > 0) {
        anyGames = true
      }
    })
    ...
    /*
      Add Wallberry-style header at the top if enabled
    */
    var wallberryHeader = null;
    if (this.config.wallberryStyle) {
      // Wallberry-style header matching WB-clock
      var hasNFL = this.config.sports.some(sport => sport.league === 'NFL');
      if (hasNFL) {
        wallberryHeader = document.createElement('div');
        wallberryHeader.classList.add('wallberry-scoreboard');
        
        // League label (like "WESTON, MA" in WB-clock)
        var leagueLabel = document.createElement('div');
        leagueLabel.classList.add('league-label');
        leagueLabel.textContent = 'NATIONAL FOOTBALL LEAGUE';
        wallberryHeader.appendChild(leagueLabel);
        
        // Week display (like time in WB-clock)
        var currentWeek = this.getNFLWeek(moment().add(this.config.debugHours, 'hours').add(this.config.debugMinutes, 'minutes'));
        var weekDisplay = document.createElement('div');
        weekDisplay.classList.add('week-display');
        weekDisplay.textContent = `WEEK ${currentWeek.weekNumber}`;
        wallberryHeader.appendChild(weekDisplay);
        
        // Matchup text (like date in WB-clock)
        if (this.sportsData['NFL'] || this.sportsData['Philadelphia Eagles']) {
          var matchupDisplay = document.createElement('div');
          matchupDisplay.classList.add('matchup-display');
          
          // Get the first game for matchup text
          var firstGame = null;
          Object.keys(this.sportsData).forEach(sport => {
            if (this.sportsData[sport]['scores'] && this.sportsData[sport]['scores'].length > 0) {
              firstGame = this.sportsData[sport]['scores'][0];
            }
          });
          
          if (firstGame) {
            var matchupText = this.supportedLeagues[firstGame.league] && this.supportedLeagues[firstGame.league].homeTeamFirst 
              ? `${firstGame.hTeamLong} vs ${firstGame.vTeamLong}`
              : `${firstGame.vTeamLong} at ${firstGame.hTeamLong}`;
            matchupDisplay.textContent = matchupText;
          }
          wallberryHeader.appendChild(matchupDisplay);
        }
        
        // Add header at the TOP of wrapper
        wrapper.insertBefore(wallberryHeader, wrapper.firstChild);
      }
    } else if (this.config.showWeekLabel) {
      // Regular week label at bottom
      var hasNFL = this.config.sports.some(sport => sport.league === 'NFL');
      if (hasNFL && (this.sportsData['NFL'] || this.sportsData['Philadelphia Eagles'])) {
        var weekLabel = document.createElement('div');
        weekLabel.classList.add('week-label');
        
        var currentWeek = this.getNFLWeek(moment().add(this.config.debugHours, 'hours').add(this.config.debugMinutes, 'minutes'));
        weekLabel.innerHTML = `<span>Week ${currentWeek.weekNumber}</span>`;
  ...
  socketNotificationReceived: function (notification, payload) {
    if (notification === 'MMM-SCOREBOARDNEW-SCORE-UPDATE' && payload.instanceId == this.identifier) {
      // Log.info('[MMM-ScoreboardNew] Updating Scores')
      this.loaded = true
    ...
    else if (notification === 'MMM-SCOREBOARDNEW-SCORE-UPDATE-YD' && payload.instanceId == this.identifier) {
      // Log.info('[MMM-ScoreboardNew] Updating Yesterday\'s Scores')
      this.loaded = true
    ...
    else if (notification === 'MMM-SCOREBOARDNEW-LOCAL-LOGO-LIST' && payload.instanceId == this.identifier) {
  ...
  start: function () {
    ...
    */
    ...
    this.config.sports.forEach(function (sport) {
      if (self.supportedLeagues[sport.league]) {
        ...
        Log.warn(`[MMM-ScoreboardNew] League ${sport.league} is not a valid league name`)
      }
    })
    ...
    */
    ...
    this.sendSocketNotification('MMM-SCOREBOARDNEW-GET-LOCAL-LOGOS', { instanceId: this.identifier })
  ...
  getNFLWeek: function (date) {
    // NFL weeks typically run Tuesday to Monday
    // This function calculates the current NFL week based on a given date
    var currentDate = moment(date)
    
    // Find the most recent Tuesday (start of NFL week)
    var dayOfWeek = currentDate.day() // 0 = Sunday, 1 = Monday, 2 = Tuesday, etc.
    var daysToSubtract = (dayOfWeek + 5) % 7 // Calculate days back to Tuesday
    if (daysToSubtract === 0 && dayOfWeek !== 2) {
      daysToSubtract = 7 // If it's Tuesday, don't go back a full week
    }
    
    var weekStart = currentDate.clone().subtract(daysToSubtract, 'days').startOf('day')
    var weekEnd = weekStart.clone().add(6, 'days').endOf('day')
    
    return {
      start: weekStart,
      end: weekEnd,
      weekNumber: this.getNFLWeekNumber(weekStart)
    }
  },

  getNFLWeekNumber: function (weekStartDate) {
    // Calculate NFL week number based on the season start
    // NFL season typically starts the first week of September
    var seasonStart = moment(weekStartDate).startOf('year')
    seasonStart.month(8) // September (0-indexed)
    seasonStart.date(1)
    
    // Find the first Tuesday of September
    while (seasonStart.day() !== 2) {
      seasonStart.add(1, 'day')
    }
    
    var weeksDiff = weekStartDate.diff(seasonStart, 'weeks')
    return Math.max(1, weeksDiff + 1) // Week 1 is the first week of the season
  },
  ...
  getScores: function () {
    ...
    if (self.config.debugHours > 0 || self.config.debugMinutes > 0) {
      Log.debug(`[MMM-ScoreboardNew] ${gameDate}`)
    }
    self.loadTime.start = Date.now()
    this.config.sports.forEach(function (sport, index) {
      // Check if this is NFL - if so, use weekly logic instead of daily
      var isNFL = sport.league === 'NFL'
      var dateKey, previousDateKey
      
      if (isNFL) {
        // For NFL, use day-based rollover logic instead of hour-based
        var currentWeek = self.getNFLWeek(gameDate)
        dateKey = currentWeek.start.format('YYYY-MM-DD')
        previousDateKey = currentWeek.start.clone().subtract(1, 'week').format('YYYY-MM-DD')
        
        // Check if we should show previous week's games based on rolloverDays
        var daysSinceWeekEnd = moment().diff(currentWeek.end, 'days');
        var shouldShowPreviousWeek = daysSinceWeekEnd <= self.config.rolloverDays;
        
        // For NFL, "today" means current week
        if (self.noGamesToday[sport.league] === dateKey) {
          whichDay.today = false
        } else {
          whichDay.today = true; // Always show current week if games exist
        }
        
        // Show previous week only if within rolloverDays
        if (shouldShowPreviousWeek && self.ydLoaded[sport.league] && !self.ydLoaded[sport.league].loaded) {
          whichDay.yesterday = 'yes'
        } else if (!shouldShowPreviousWeek) {
          whichDay.yesterday = 'no' // Don't show if past rollover period
        } else {
          whichDay.yesterday = tempYesterday
        }
      } else {
        // For all other sports, use daily logic (existing behavior)
        dateKey = gameDate.format('YYYY-MM-DD')
        previousDateKey = gameDate.clone().subtract(1, 'day').format('YYYY-MM-DD')
        ...
        if (self.ydLoaded[sport.league] && self.ydLoaded[sport.league].loaded && self.ydLoaded.date === dateKey) {
      ...
      var payload = {
        ...
        label: thisLabel,
        gameDate: isNFL ? self.getNFLWeek(gameDate).start : gameDate,
        whichDay: whichDay,
        ...
        isWeekly: isNFL // Add flag to indicate weekly mode
      }

      self.sendSocketNotification('MMM-SCOREBOARDNEW-GET-SCORES', payload)
    })
  },
