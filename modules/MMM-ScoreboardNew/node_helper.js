...
module.exports = NodeHelper.create({
  ...
  start: function () {
    ...
    this.localLogos = {}
    var fsTree = this.getDirectoryTree('./modules/MMM-ScoreboardNew/logos')
    ...
    fsTree = this.getDirectoryTree('./modules/MMM-ScoreboardNew/logos_custom')
    fsTree.forEach((league) => {
      if (league.children) {
        var logoFiles = []
        league.children.forEach((file) => {
  ...
  socketNotificationReceived: function (notification, payload) {
    if (notification == 'MMM-SCOREBOARDNEW-GET-SCORES') {
      /*
        payload contains:
      ...
      */
      ...
      if (payload.whichDay.today) {
        ...
        if (payload.isWeekly && payload.league === 'NFL') {
          self.getNFLWeeklyScores(payload, provider, function (scores, sortIdx) {
            self.sendSocketNotification('MMM-SCOREBOARDNEW-SCORE-UPDATE', { instanceId: payload.instanceId, index: payload.league, scores: scores, label: payload.label, sortIdx: sortIdx })
          })
        } else {
        ...
        self.sendSocketNotification('MMM-SCOREBOARDNEW-SCORE-UPDATE', { instanceId: payload.instanceId, index: payload.league, scores: [], notRun: true, label: payload.label, sortIdx: 999 })
      }
      if (payload.whichDay.yesterday === 'yes') {
        ...
        if (payload.isWeekly && payload.league === 'NFL') {
          self.getNFLWeeklyScores(payload, provider2, function (scores, sortIdx) {
            self.sendSocketNotification('MMM-SCOREBOARDNEW-SCORE-UPDATE-YD', { instanceId: payload.instanceId, index: payload.league, scores: scores, label: payload.label, sortIdx: sortIdx })
      ...
      /* else if (payload.whichDay.yesterday === 'erase') {
        ...
        self.sendSocketNotification('MMM-SCOREBOARDNEW-SCORE-UPDATE-YD', { instanceId: payload.instanceId, index: payload.league, scores: [], label: payload.label, sortIdx: 999 })
      } */
    }
    else if (notification == 'MMM-SCOREBOARDNEW-GET-LOCAL-LOGOS') {
      this.sendSocketNotification('MMM-SCOREBOARDNEW-LOCAL-LOGO-LIST', { instanceId: payload.instanceId, index: payload.league, logos: this.localLogos, logosCustom: this.localLogosCustom })
    }
  },
